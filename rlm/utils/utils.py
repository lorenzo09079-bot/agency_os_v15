...
def find_final_answer(response: str) -> Optional[tuple]:
    ...
    if final_match:
        ...
        for wrapper in [("'", "'"), ('"', '"'), ("f'''", "'''"), ("f'", "'"),
                       ('f"', '"'), ("'''", "'''"), ('"""', '"""')]:
            if content.startswith(wrapper[0]) and content.endswith(wrapper[1]):
                content = content[len(wrapper[0]):-len(wrapper[1])]
                break
    ...
    if answer_type == 'FINAL':
        ...
        try:
            ...
            if variable_name in repl_env.locals:
                ...
                error_msg = f"Variabile '{variable_name}' non trovata. Disponibili: {available}"
...
def add_execution_result_to_messages(messages, code, result, max_length=200000):
    """Aggiunge risultato esecuzione ai messaggi. Stdout NON troncato (fino a 200K)."""
